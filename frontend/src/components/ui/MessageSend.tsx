...
export const MessageSend = ({ onClick, text, setText }: Props) => {
  const handleClick = () => {
    onClick();
    setText("");
  };
  return (
    <div className="p-3 flex gap-3 items-center bg-gray-50">
      <svg
        className="w-6 h-6 text-gray-500 flex-grow"
        aria-hidden="true"
      ...
      <input
        type="text"
        placeholder="話したいことを書いてみよう…"
        className="px-2 py-2.5 rounded-md placeholder:text-sm"
        value={text}
        onChange={(e) => setText(e.target.value)}
      />
      <button type="button" onClick={handleClick}>
        <svg
          className="w-6 h-6 text-blue-700 rotate-90 flex-grow"
          aria-hidden="true"
          xmlns="http://www.w3.org/2000/svg"
          width="24"
          height="24"
